...
BEGIN_MESSAGE_MAP(CThreadTestDlg, CDialog)
	...
	ON_BN_CLICKED(IDCANCEL, &CThreadTestDlg::OnBnClickedCancel)
END_MESSAGE_MAP()
...
UINT CThreadTestDlg::mthreadRunner( LPVOID lpvoid ){
    HWND tmpHWND = *( HWND *) lpvoid;

    CString tmpStr;

    for ( int i = 0 ; i < 100 ; i++ ){
        tmpStr.Format( _T("%g") , i + 0.5 );
...
void CThreadTestDlg::OnBnClickedCancel()
{
	...
	/*TerminateThread(pCWinThread_->m_hThread, 1);
	Sleep(10);
	pCWinThread_ = NULL;*/

	/*int dwRes = WaitForSingleObject(pCWinThread_->m_hThread, 1000L);
	switch (dwRes)
	{
	case WAIT_OBJECT_0:
		TerminateThread(pCWinThread_->m_hThread, 1);
